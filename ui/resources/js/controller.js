...
function checkPage() {
  ...
  if (currentPage.indexOf(URLS.room_type) > 0) {
    if (!$.isEmptyObject(DATA.room_type)) {
      ...
      selectRoomType(
        $(".image-group[data-index=" + DATA.room_type["id"] + "]")
      );
    }
...
function move(path) {
  window.location.href = window.location.origin + "/views/" + path + ".html";
}
...
function generateRoomTypeSelection() {
  ...
  $selection.on("change", function() {
    // console.log(this);
    selectRoomType(
      $(".image-group[data-index=" + this.selectedOptions[0].value + "]")
    );
  });
}
